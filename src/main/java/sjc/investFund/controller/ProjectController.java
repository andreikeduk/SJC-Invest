...
public class ProjectController {
	...
	@RequestMapping(value = "/{id}/sendMoneyFromArea", method = RequestMethod.GET)
	public String sendMoneyFromArea(@PathVariable("id") Project project,
			Model model) {

		model.addAttribute("transaction", new Transaction());
		model.addAttribute("action");

		return "sendMoneyFromArea";
	}

	@PreAuthorize("hasRole('ROLE_DIRECTOR')")
	@RequestMapping(value = "/{id}/sendMoneyFromArea", method = RequestMethod.POST)
	public String sendMoneyFromArea(@PathVariable("id") Project project,
			@ModelAttribute("transaction") @Valid Transaction transaction,
			BindingResult bindingResult, HttpSession session, Model model) {
		String view = "redirect:/redirector";
		if (bindingResult.hasErrors()) {
			view = "sendMoneyFromArea";
		} else {
			transaction.setGoalAccount(project.getAccount());
			transaction.setInvestorAccount(project.getArea().getAccount());
			transactionService.saveTransaction(transaction);
			model.asMap().remove("transaction");
		}
		return view;
	}
